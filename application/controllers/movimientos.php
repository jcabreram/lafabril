...
class Movimientos extends CI_Controller
{
	...
	public function crear_reporte_cartera()
	{
		...
		if ($this->form_validation->run()) {
			...
			$this->_makeWalletReport($filters);
		}
	...
	private function _makeWalletReport($filters)
	{
		...
		if (count($clients) === 0) {
			...
			redirect("movimientos/crear_reporte_cartera");
		}
	...
	public function crear_reporte_antiguedad()
	{
		// Load necessary models
		$this->load->model('userBranches');
		$this->load->model('clients');
		...
		// Display views
		$this->load->view('header', $data);
		$this->load->view('movimientos/crear_reporte_antiguedad', $data);
		$this->load->view('footer', $data);
	}

	private function _makeAntiquityReport($filters)
	{
		$this->load->helper(array('dompdf', 'file'));
		$this->load->model('branches');
		$this->load->model('clients');
		...
		if (count($clients) === 0) {
			...
			redirect("movimientos/crear_reporte_antiguedad");
		}
